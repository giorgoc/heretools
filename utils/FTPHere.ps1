...
$configDir = Join-Path $env:LOCALAPPDATA "FTPHere"

function Write-Header {
    Write-Host ""
    Write-Host "  FTP HERE v$Version" -ForegroundColor Cyan
    Write-Host "  Instant temporary FTP sharing" -ForegroundColor DarkGray
...
try {
    ...
    Write-Host "  ftp://$bindIP`:$port" -ForegroundColor Green
    ...
    # ------------------------------------------------------------
    # Bind FTP service only to selected interface
    # ------------------------------------------------------------

    $env:SFTPGO_FTPD__BINDINGS__0__ADDRESS = $bindIP
    ...
    try {
        ...
        Remove-Item `
            Env:SFTPGO_FTPD__BINDINGS__0__ADDRESS `
            -ErrorAction SilentlyContinue
    }
}
catch {
